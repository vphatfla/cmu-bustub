...
| Task #4 | External Merge Sort + Limit Executors + Window Functions (ExternalMergeSort ✅, Limit ✅, TupleComparator+GenerateSortKey ✅, WindowFunction ✅) | ✅ DONE — p3.16, p3.18, p3.19, p3.20 all pass (regression check completed 2026-08-11) |
...
- **Task #4**: re-verified 2026-08-03 (fresh parallel-agent read of every file, not cache — corrects the stale "uncommitted" notes below). **`limit_executor.{h,cpp}` is fully implemented and COMMITTED** (commit `26f6a87`, "finish limit executor and tuple comparator") — ctor stores `plan_`/`child_executor_`; `Init()` re-inits child, clears `child_tuples_`/`child_rids_` buffers, resets `limit_ = plan_->GetLimit()`; `Next()` refills the buffer from the child in `BUSTUB_BATCH_SIZE` chunks and copies into the output batch while `batch_size>0 && limit_>0`, decrementing both. **`execution_common.cpp` (`TupleComparator::operator()` + `GenerateSortKey`) is also fully implemented, verified correct, and COMMITTED** (same commit `26f6a87`) — see the dedicated "TupleComparator + GenerateSortKey — DONE" subsection under "Task #4 — Code Recon" for the full null-handling bug history (went through 4 iterations before landing on a correct, antisymmetric implementation). Everything else confirmed still stubbed as of 2026-08-03: `topn_executor.cpp` (empty ctor — doesn't even store `plan_`/`child_executor_` despite both being declared as members, `NotImplementedException`, `GetNumInHeap` throws), `sort_limit_as_topn.cpp` (returns plan unchanged, no recursion into children), `topn_per_group_executor.cpp` (ctor stores plan+child, `NotImplementedException`), `window_function_executor.cpp` (ctor stores plan+child, `Init()` throws, `Next()`→false). Full per-file breakdown with exact stub code and design notes in "Task #4 — Code Recon" section below.
- **`ExternalMergeSortExecutor` — ✅ DONE and COMMITTED** (commit `b616b68`, "finish external merge sort"), verified 2026-07-26: `p3.16-sort-limit.slt` passes completely (exit 0, zero `wrong result` occurrences), including the 100,000-row `__mock_external_merge_sort_input` table, multi-column `ORDER BY` with mixed `ASC`/`DESC`, all `NULLS FIRST`/`NULLS LAST`/default combinations, and nested subqueries with `LIMIT`. Full implementation details and bug history (10 distinct bugs found and fixed via pair-programming review) in the dedicated "ExternalMergeSort" section below.
- **CORRECTION (2026-08-03) — TopN is NOT part of the official Fall 2025 Task #4 spec.** Fetched `https://15445.courses.cs.cmu.edu/fall2025/project3/#task4` directly (WebFetch TLS-fails on this host — used `curl -sL` instead) and confirmed the spec's own "You will complete your implementation in the following files" list for Task #4 is exactly: `intermediate_result_page.h`, `execution_common.cpp`, `external_merge_sort_executor.{h,cpp}`, `limit_executor.{h,cpp}`, `window_function_executor.{h,cpp}`. Zero occurrences of "top"/"TopN" anywhere on the page, including in the Optional Leaderboard section (Query 1 = hash-join filter decomposition + pushdown, Query 2 = column pruning + predicate pushdown + always-false-filter elimination, Query 3 = bloom filter for hash join build side — none involve sorting/top-N). **`topn_executor.cpp`, `sort_limit_as_topn.cpp`, and `topn_per_group_executor.cpp` are present in the repo (with matching `.slt` tests) but are NOT required for Task #4 credit per this semester's graded spec** — likely stub/test infra carried over in the BusTub skeleton from another semester, not investigated further. **Corrected next (and final) milestone for full Task #4 credit: `WindowFunctionExecutor` only.** Do not resume TopN work based on older notes in this file without re-confirming against the live spec page first.
...
### Untracked components discovered — confirmed OUT OF SCOPE for Task #4 credit (2026-08-03)
- **TopN**: `src/execution/topn_executor.cpp` (has `GetNumInHeap()`) + `src/optimizer/sort_limit_as_topn.cpp` (Sort+Limit→TopN rule). Tested by `p3.17-topn.slt`. **Confirmed NOT in the official Fall 2025 Task #4 required-files list** (see correction under "Verified State" above) — leave stubbed unless deliberately pursuing this as bonus/extra work.
- **TopNPerGroup**: `src/execution/topn_per_group_executor.cpp`. Tested by `p3.leaderboard-q1/q1-index/q1-window/q2/q3.slt`. Also not named anywhere in the spec's Optional Leaderboard section (which describes 3 join-focused optimizations, not top-N-per-group) — likely unrelated skeleton infra, not investigated further.
...
Phase 4 (Task #4):  Limit ✅ → TupleComparator ✅ → ExternalMergeSort ✅ → WindowFunction (next, and FINAL required file — TopN/sort_limit_as_topn/TopNPerGroup are NOT in the official Task #4 spec, see 2026-08-03 correction above)
```
...
- **Design**: STREAM the LEFT (outer) side one buffered batch at a time in `Next()`; materialize the RIGHT (inner) side per left-batch (see `nlj_init_check` gotcha below for why — NOT once in `Init()`). Left=outer=child0=`GetLeftPlan()`, right=inner=child1=`GetRightPlan()` (fixed by planner; factory passes them in that order). NLJ is NOT graded on memory (only HashJoin/ExternalMergeSort spill), so materializing the inner is fine even if it's the larger table.
  - **Correction (2026-08-11, verified against actual source)**: `Init()` itself (`nested_loop_join_executor.cpp:48-60`) does NOT materialize the right side at all — it only calls `right_executor_->Init()` and clears buffers. All right-side population happens exclusively inside `Next()`'s left-refill block, per the `nlj_init_check` gotcha documented below. The line above describing "materialize RIGHT fully in Init()" was inaccurate in earlier revisions of this doc.
...
**Suggested Task #4 order** (easiest → hardest, matches dependency chain): `Limit` ✅ done → `TupleComparator`+`GenerateSortKey` ✅ done → `ExternalMergeSort` ✅ done (`p3.16-sort-limit.slt` passes) → **`WindowFunction` (next and FINAL required file for Task #4 credit)**. `TopN`/`sort_limit_as_topn`/`TopNPerGroup` are confirmed NOT part of the official Fall 2025 Task #4 spec (verified 2026-08-03 directly against the course page) — skip unless deliberately chasing bonus/out-of-scope work. Tests for the required path: `p3.16-sort-limit` ✅, `p3.18/19-integration`, `p3.20-window-function`. (`p3.17-topn` and `p3.leaderboard-*` exercise the out-of-scope files.)

### `WindowFunctionExecutor` — 🔶 IN PROGRESS, recon done 2026-08-09 (user has started writing `Init()`; pair-programming — user writes, Claude reviews)

#### `window_plan.h` — confirmed structure (read fresh, not from cache)
```cpp
enum class WindowFunctionType { CountStarAggregate, CountAggregate, SumAggregate, MinAggregate, MaxAggregate, Rank };
struct WindowFunction {
  AbstractExpressionRef function_;             // SINGULAR — one expr per window function, not a vector
  WindowFunctionType type_;
  std::vector<AbstractExpressionRef> partition_by_;
  std::vector<OrderBy> order_by_;              // OrderBy from binder/bound_order_by.h
};
// WindowFunctionPlanNode public members (no getters — access directly):
std::vector<AbstractExpressionRef> columns_;                       // ALL output cols: real selected cols + placeholders for window-func slots
std::unordered_map<uint32_t, WindowFunction> window_functions_;    // key = output column index of the placeholder
```
**Critical: each `WindowFunction` has its OWN independent `partition_by_`/`order_by_`.** Confirmed by `p3.20-window-function.slt` query 7 (file's own comment: *"different window functions should calculate separately"*): `select sum(v2) over (partition by v1 order by v2), sum(v2) over (order by v2) from t2;` — column 1 partitions by `v1`+orders by `v2`; column 2 has NO partition, orders by `v2` globally. **A single shared/"global" sort pass across all window functions in the plan is incorrect** — each `WindowFunction` entry in the map must be processed with its own partition grouping + its own sort order, independently of every other entry.

#### Frame semantics, confirmed against actual `p3.20-window-function.slt` expected rows
- **No `ORDER BY` on that window function** → frame = whole partition (or whole table if no `PARTITION BY` either); the SAME aggregate value is repeated on every row of the partition (query 1: 6 identical rows `6 -99999 99999 6 6`; query 5: partition `v1=1` gets `3 100 300 3 600` on all 3 of its rows).
- **`ORDER BY` present on that window function** → frame = UNBOUNDED PRECEDING to CURRENT ROW, i.e. a running/cumulative aggregate that **resets at each partition boundary** (query 6: partition `v1=2`'s running sum restarts at `400`, doesn't continue from partition `v1=1`'s `600`).
- **`RANK()`**: standard SQL semantics, verified via duplicate-value test data in query 3/4 — tied rows (equal ORDER BY key) share the same rank; the next distinct rank **skips** by the tie-group size (two rows tied at rank 3 → next used rank is 5, not 4). Not dense_rank.
- Output must preserve the CHILD's original row order (window functions annotate rows, never reorder/drop them) — need to track original tuple position separately from whatever order a per-window-function sort computes.

#### Reusable aggregate machinery — lives in `aggregation_executor.h`, NOT `aggregation_plan.h`
`AggregateKey{group_bys_}` / `AggregateValue{aggregates_}` (with NULL-aware `==`/hash) are declared in `aggregation_plan.h`, but `SimpleAggregationHashTable`, `MakeAggregateKey`/`MakeAggregateValue`, `GenerateInitialAggregateValue()`, and `CombineAggregateValues()` (the actual per-type Count/CountStar/Sum/Min/Max combine arithmetic) are all in `src/include/execution/executors/aggregation_executor.h`. `CombineAggregateValues` operates over **parallel vectors** `agg_exprs_`/`agg_types_` (multi-column aggregation) — since each `WindowFunction` here is single-expr/single-type, either (a) construct one single-element `SimpleAggregationHashTable` per distinct `WindowFunction`, or (b) hand-roll the same switch-based combine logic inline for one value at a time. `RANK` has no analog in `AggregationType` — needs separate positional logic (not a combinable running value in the same sense).

#### Final implementation (`window_function_executor.{h,cpp}`) — ✅ DONE, `p3.20-window-function.slt` passes end-to-end (verified 2026-08-09, exit 0, zero `wrong result` occurrences, no crash)
Built via pair-programming (user wrote the code, Claude reviewed/verified after each edit) — same workflow as ExternalMergeSort/HashJoin. Several real bugs found and fixed across review passes; kept here in case of regression.

**Data structures** (all members on `WindowFunctionExecutor`):
- `child_tuples_` (`vector<Tuple>`) — full child drain, original scan order preserved, index = row identity.
- `postion_tracker_` (`size_t`) — `Next()`'s streaming cursor, reset to 0 in `Init()`.
- `presentation_index_order_` (`vector<size_t>`) — the ORDER (by original index) that `Next()` emits rows in. See "presentation order" below — this is the trickiest part of the whole executor.
- `window_func_results_` (`unordered_map<uint32_t, vector<Value>>`) — keyed by output column index (matches `window_functions_`'s key type exactly); each value vector is sized to `child_tuples_.size()` and indexed by ORIGINAL row index, i.e. `window_func_results_[col_idx][original_idx]`. Only window-function output columns get an entry — plain selected columns are evaluated fresh in `Next()` instead of precomputed (no need to store them ahead of time).
- Two small private helpers, `InitBucketValue(WindowFunctionType) const -> Value` and `CombineValueOverBucket(WindowFunctionType, Value*, const Value&) const` — hand-written scalar mirrors of `SimpleAggregationHashTable::GenerateInitialAggregateValue`/`CombineAggregateValues` (from `aggregation_executor.h`). **Deliberately NOT reusing the real `SimpleAggregationHashTable` class** — considered and rejected: its constructor holds `agg_exprs_`/`agg_types_` by reference (would need lifetime-managed wrapper vectors per window function), it takes `AggregationType` not `WindowFunctionType` (two independently-declared enum classes — same first 5 names, but converting between them needs an unsafe `static_cast` with no compiler-enforced link), and `AggregationType` has no `Rank` case at all so a custom path is unavoidable regardless. The ~15-line hand-rolled switch was judged lower-risk than that reuse path.

**Algorithm, per entry in `plan_->window_functions_` (each processed fully independently — confirmed necessary by `p3.20` query 7's "different window functions should calculate separately" comment, where two window functions in one query have different `partition_by_`/`order_by_`):**
1. **Partition**: bucket original tuple indices by `partition_by_` using `AggregateKey` (reused as-is from `aggregation_plan.h` — it's just a hashable/equality-correct wrapper around a row's evaluated grouping-expression values, with NULL-safe equality; the exact same shape problem `GROUP BY` already solved, so no new "PartitionKey" type needed). Empty `partition_by_` → single bucket covering all rows (matches "no PARTITION BY = whole table is one partition").
2. **Per bucket** (`for (auto &[key, indices] : partitions)`):
   - **No `order_by_`** (Case A — whole-partition/broadcast): one combine pass over `indices` via `CombineValueOverBucket`, then broadcast the single final value to every index in the bucket. `continue`s before touching sort logic at all.
   - **`order_by_` present**: build `sorted_bucket: vector<pair<size_t, SortKey>>` (index paired with its precomputed `GenerateSortKey` result — computed once per index, not recomputed inside the sort comparator), sort once via `TupleComparator{order_by_}`. Then:
     - **`Rank`**: walk `sorted_bucket`, 1-indexed position = new rank UNLESS tied with the previous entry (tie test = `!cmp(a,b) && !cmp(b,a)`, i.e. "neither less than the other" — reuses the already-NULL-safe `TupleComparator` instead of writing a new equality function). `running_rank > 0` guards the `sorted_bucket[i-1]` lookback from underflowing at `i=0`.
     - **Everything else (Count/CountStar/Sum/Min/Max)** (Case B — running/cumulative): walk `sorted_bucket`, fold each row into a running accumulator via `CombineValueOverBucket`, write the accumulator to `window_func_result[idx]` at EVERY step (not just the end) — this is what makes it a running/cumulative frame instead of whole-partition.

**Presentation order — the one genuinely surprising design point, confirmed by re-reading `p3.20-window-function.slt` directly rather than assuming:** `Next()` does NOT emit rows in original child scan order. Query 2 (`order by v1` shared by all 5 window functions, checked with a strict non-`rowsort` `query` block) proves this — `t1`'s insertion order is `(-99999, 99999, 0, 1, 2, 3)` but the expected output is in **v1-ascending sorted order** (`-99999, 0, 1, 2, 3, 99999`), confirmed by tracing the running max column in the expected output. Every OTHER query with multiple/divergent window-function specs uses `query rowsort`, meaning row order isn't checked there at all — order only matters in the one degenerate case where every window function in the query happens to share an identical spec (so any one of them's sorted traversal IS the unambiguous correct order). Implementation: capture `presentation_index_order_` from the FIRST window function encountered that has a non-empty `order_by_` (arbitrary choice among ties is fine — `rowsort` protects every case where it would matter); `Next()` reads `presentation_index_order_[postion_tracker_]` instead of `postion_tracker_` directly, falling back to plain original order when no window function has any `order_by_` at all.
- **Bug fixed (caught before running, confirmed via source-read not assumption): `Value::Add`/`Min`/`Max` are `const`, return-by-value, do NOT mutate `*this`** (confirmed by reading `type/value.h`: `inline auto Add(const Value &o) const -> Value { return Type::GetInstance(type_id_)->Add(*this, o); }`). First draft of `CombineValueOverBucket` called these as bare statements (`out_value->Add(...)`) and discarded the return — every window aggregate type would have gotten stuck at its initial seed value forever (CountStar always 0, Sum/Count/Max/Min never updating past the first row). Fixed by assigning back: `*out_value = out_value->Add(...)` (and `Max`/`Min` equivalents) in all 5 switch cases.
- **Bug fixed: `Next()` originally evaluated EVERY `plan_->columns_[i]` expression directly against the child tuple, including window-function placeholder columns** — the class's own docstring warns this is invalid ("Directly use placeholders for window function columns in columns is not allowed, as it contains invalid column id"). Fixed by checking `window_func_results_.find(col_idx)` first (falls back to `Evaluate()` only for plain columns not present in the map).
- **Bug fixed: `rid_batch` was never populated** in `Next()` (only `tuple_batch` grew) — broke the tuple/RID batch size-parity invariant every other executor in this codebase maintains. Fixed with `rid_batch->emplace_back(RID{});` alongside each emitted tuple.
- **Bug fixed (found by an actual test run, not hand-tracing): `presentation_index_order_.empty()` was originally checked INSIDE the per-partition-bucket loop, not once per window function.** For a window function with more than one partition (e.g. `partition by v1 order by v2` on `t2`, 2 partitions), the first bucket processed appended its indices and made the vector non-empty — so the SECOND bucket's indices were silently never appended, leaving `presentation_index_order_` shorter than `child_tuples_.size()`. `Next()`'s loop bound is `child_tuples_.size()`, so `presentation_index_order_[postion_tracker_]` read past the end of the truncated vector once `postion_tracker_` exceeded its real size — `std::vector::operator[]` has no bounds check, so this was a genuine out-of-bounds read, and manifested as a **segfault (exit code 139)** confirmed by an actual run of `p3.20`, not just reasoning. Fixed by hoisting the decision to a `const bool should_capture_presentation_order = presentation_index_order_.empty() && !window_func.order_by_.empty();` computed ONCE per window function (before its per-partition loop), so a single function's buckets either ALL contribute or NONE do — never a partial contribution.
- Minor fix: `InitBucketValue`'s `const` was originally placed on the RETURN TYPE (`-> Value const`) instead of as a member-function qualifier — this doesn't express "this method doesn't mutate `*this`" (that requires `) const -> Value`, i.e. const AFTER the parameter list); it only blocks move-construction at call sites, forcing an extra copy. Fixed to `auto InitBucketValue(WindowFunctionType wft) const -> Value;`.

**Lesson for next time a per-item "have I captured X yet" flag gates work inside a nested loop**: the flag must be decided at the same loop level as what constitutes "a complete X" — checking `.empty()` (or any partial-progress signal) INSIDE a loop whose full iteration is required to produce one complete unit will silently truncate that unit the moment ANY progress has been made, not just when it's actually finished. Same root-cause shape as the ExternalMergeSort session's "local variable is not a substitute for the container element" lesson — a flag/check needs to live at the scope that matches the unit of work it's actually gating.

**Regression check completed 2026-08-11** (parallel-agent verification, fresh source read + real test runs, not cache): `p3.16-sort-limit.slt`, `p3.18-integration-1.slt`, `p3.19-integration-2.slt` (note: the actual filenames are `-1`/`-2` suffixed, not the bare `p3.18-integration.slt`/`p3.19-integration.slt` named earlier in this doc), and `p3.20-window-function.slt` all pass — exit 0, zero `wrong result` occurrences on every file. **Task #4 is fully complete**; all required Task #4 files (`intermediate_result_page.h`, `execution_common.cpp`, `external_merge_sort_executor.{h,cpp}`, `limit_executor.{h,cpp}`, `window_function_executor.{h,cpp}`) verified implemented and passing.

### Autograder (Release build) failures found AFTER the above regression check — 3 real bugs, all fixed 2026-08-11
The actual grading harness runs from `rel_build` (**Release**, `-DNDEBUG`), not the `Debug` config used for all local testing all session. Debug-only testing had been hiding two real bugs and one design flaw that only manifest under Release + a memory-constrained `--bpm-size`. Found via a long empirical bisection (isolate query → count rows → sweep bpm-size → UBSan → per-page write/read instrumentation), documented here in case of regression.

1. **`ExternalMergeSortExecutor`'s P0 chunk size was a hardcoded literal, blowing the grader's disk-write budget.** The grader invokes `p3.16-sort-limit.slt` with `--bpm-size 8 --check-min-disk-write 2250 --check-max-disk-write 7000` (no `--in-memory` — real on-disk `DiskManager`). With `BUSTUB_BATCH_SIZE` (20) as the run-generation chunk size, the file's 100,000-row `__mock_external_merge_sort_input` table (sorted twice in that file) alone produced **35,152 disk writes** — 5x over budget (~5000 initial runs, ~13 merge rounds). **Fix**: `Init()` (`external_merge_sort_executor.cpp`) now sizes each P0 chunk off a BYTE budget derived from `exec_ctx_->GetBufferPoolManager()->Size()` (`(bpm_size - 2) * BUSTUB_PAGE_SIZE`, mirroring the classic "B-1 buffer pages for the initial run" external-sort design) instead of a fixed tuple count — adapts correctly across whatever `--bpm-size` the grader uses for any file, not just an empirically-tuned magic number. Implementation accumulates tuples across repeated `BUSTUB_BATCH_SIZE`-granularity child `Next()` calls into a growing chunk, flushing (sort + spill as one run) once the byte budget is hit or the child is exhausted — restructured as `while (true) { ...; if (!child_has_more) break; }` (not a `while (child_has_more)` loop with the reassignment inside) specifically because clang-tidy's `bugprone-infinite-loop` check flagged the reassign-inside-loop-condition form as a false positive.

2. **`ArcReplacer::RecordAccess` had a real divide-by-zero → float-to-unsigned UB**, `src/buffer/arc_replacer.cpp:112-113` (Project 1 code, not P3). When adjusting `mru_target_size_` on an `mru_ghost_` hit, if `mru_ghost_` is empty (and `mfu_ghost_` isn't), `std::floor(static_cast<double>(mfu_ghost_.size()) / mru_ghost_.size())` divides by zero → `+inf` (well-defined for floating point), then `mru_target_size_ += delta` converts that `inf` double into `mru_target_size_` (a `size_t`) — **converting an out-of-range float to an unsigned integer is UB**, confirmed directly by UBSan: `1.84467e+19 is outside the range of representable values of type 'unsigned long'` (≈2⁶⁴). This ghost-list-empty state essentially never occurs under a generous buffer pool (why Project 1's own tests and all prior P3 debug-build testing never hit it) but is routine under `--bpm-size 8` cycling ~200 pages through 8 frames. Once corrupted, every subsequent `Evict()` decision (which reads `mru_target_size_`) is steering off garbage. **Fix**: added `else if (mru_ghost_.empty()) { mru_target_size_ = replacer_size_; }` (saturate instead of dividing by zero) and the symmetric guard for the `mfu_ghost_`-empty branch (which was already accidentally shielded by an `if (mru_target_size_ < delta)` clamp, but is now guarded explicitly too for clarity/robustness rather than relying on that incidental behavior).
   - **This fix alone did NOT resolve the grader failures** — after applying it, `p3.16`'s isolated 100k-row sort still lost 1136/100000 rows under `--bpm-size 8`, and (surprisingly) still lost 195/100000 rows even at `--bpm-size 300` (far more frames than the ~196 pages needed, i.e. zero eviction) — proving there was a SEPARATE bug independent of eviction/ArcReplacer entirely. Bug #3 below is that second, dominant bug.

3. **THE REAL ROOT CAUSE — `BUSTUB_ASSERT(page->InsertTuple(tuple), "...")` silently discarded the insert in Release builds.** `BUSTUB_ASSERT(expr, msg)` expands to `assert((expr) && (msg))` (`common/macros.h:23`), and CMake's `Release` build type defines `NDEBUG` by default, which makes `assert()` a **complete no-op** — not just skipping the check, but **never evaluating `expr` at all**. Both `MergeSortRun::InsertTuple` (`external_merge_sort_executor.h`) and `HashJoinExecutor::InsertTupleIntoPartition` (`hash_join_executor.cpp:368`, same `IntermediateResultPage`-based design) had the pattern `BUSTUB_ASSERT(page->InsertTuple(tuple), "New page insert must success");` for the first tuple written to every newly-allocated page — in Release, `page->InsertTuple(tuple)` (the actual write) never ran, silently losing exactly the first tuple of every page except the very first page of a run (which is inserted via a separate, unconditional call). This is the classic "never put side effects inside `assert()`" pitfall. Confirmed via per-page write/read instrumentation (write count via an external counter vs. `IntermediateResultPage::GetNumTuples()` read back showed e.g. "written=512 read=511" for every page after the first) and a direct print showing `GetNumTuples()==0` immediately after a supposedly-successful insert onto a fresh page. Not UB (UBSan found nothing here — `assert()`-as-no-op-under-`NDEBUG` is explicitly well-defined by the standard), just a plain logic bug that only a Release build (or `-DNDEBUG`) exposes. **Fix, applied identically in both files**: split into `bool inserted = page->InsertTuple(tuple); BUSTUB_ASSERT(inserted, "...");` so the insert always executes regardless of `NDEBUG`. Swept the whole codebase for the same anti-pattern (`grep -rn "BUSTUB_ASSERT(" | grep -E "\.[A-Za-z_]+\("`) — no other instances found; every remaining `BUSTUB_ASSERT` wraps a pure check (pointer/size/enum comparisons) with no side effects, safe to compile out.

**Verified 2026-08-11**: built a separate Release config (`-DCMAKE_BUILD_TYPE=Release -DBUSTUB_SANITIZER=`) specifically to reproduce the grader's `rel_build` environment (previously all testing this project used Debug only) — after all 3 fixes, the full p3 suite (00/05/06/07/10/13/14/15/16/18/19/20) passes with zero `wrong result` under Release, the exact `p3.16` grading command (`--bpm-size 8 --check-min-disk-write 2250 --check-max-disk-write 7000`) passes with exit 0, and a `--bpm-size` sweep (4/8/16/32/300) across p3.14/15/16/19 all pass. **Lesson for future sessions: always test P3/P4 work in a Release build (or at least `-DNDEBUG`) before considering it done — Debug-only testing can hide `assert()`-with-side-effects bugs and other `NDEBUG`-sensitive issues indefinitely.**
...
**Fix needed**: Add a recursive helper that walks nested OR trees and collects all `col=const` equalities into a single `pred_keys` vector. Must verify all leaves reference the same column. If any leaf is not a valid equality or references a different column, bail out (keep as SeqScan).

**File**: `src/optimizer/seqscan_as_indexscan.cpp`

**Correction (2026-08-11, verified against actual source, not this TODO's original plan)**: the implemented recursive helper is named `ResolveExpr` (`seqscan_as_indexscan.cpp:69-90`), NOT `FlattenOrExpr` as originally planned above — it recurses through nested `LogicType::Or` trees via `std::all_of` over `logic_expr->children_`. The leaf splitter is `SplitComparisonExpr` (`:38-62`). Functionally matches the fix description; only the name differs. `p3.05-index-scan-btree.slt` confirmed passing end-to-end (exit 0, zero failures) as of this date.

---

## Known Issues

### `make check-clang-tidy-p3` — pre-existing noise, not caused by WindowFunctionExecutor (checked 2026-08-09)
Running this surfaces ~40 errors, but none are new/unique to `window_function_executor.{h,cpp}`. Three buckets, none blocking:
1. **Course-skeleton files** not written for P3: `catalog.h`, `value_factory.h`, `arc_replacer.h`, `b_plus_tree.h`/`.cpp`, `index_iterator.h`, `stl_ordered.h`, `stl_unordered.h`, `rid.h`, `common/macros.h`, `b_plus_tree_debug.h`, plus `fmt::formatter` template warnings inside `third_party/fmt`.
2. **A pre-existing, codebase-wide false positive**: `error: invalid case style for variable 'Next' [readability-identifier-naming]` fires on `Next()`'s out-of-line definition in EVERY executor — not just `window_function_executor.cpp:157`, but identically in `delete_executor.cpp`, `insert_executor.cpp`, `seq_scan_executor.cpp`, `aggregation_executor.cpp`, `hash_join_executor.cpp`, `external_merge_sort_executor.cpp`, `limit_executor.cpp`, etc. (all already-completed, already-passing files). clang-tidy is misclassifying the mandated `AbstractExecutor::Next` override name as a "variable." Confirmed present before this session's work, not introduced by it.
3. **Looks like a clang-tidy tooling/include-path issue, not a real defect**: `error: 'memory' file not found` in `seqscan_as_indexscan.cpp` (already-completed Task #1 file) and `error: 'cstddef' file not found` in `window_function_executor.h` — standard library headers "not found" by clang-tidy while the actual `make` build compiles clean is a strong signal of a broken header search path in the clang-tidy invocation itself (common after macOS SDK/toolchain updates), not missing includes.

Decision (user-confirmed 2026-08-09): leave as-is rather than editing skeleton files or renaming `Next`/`Init` overrides across every executor — out of scope, high blast radius on previously-graded working code, doesn't reflect an actual regression from window function work.
