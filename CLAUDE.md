...
### HashJoin — `Next()` implemented + compiles clean; BLOCKED on `nlj_as_hash_join.cpp` before any test can run — RESUME HERE
> **Verified 2026-07-18, end of session.** `Init()` (partition/repartition) and `Next()` (stream+probe+emit) are both fully written and reviewed line-by-line across several rounds; `make -j sqllogictest` builds green with zero errors. **Nothing has been run yet** — see blocker below, that's the very next thing to unblock.

#### ⚠️ Blocker confirmed by reading source, not assumption: no test can exercise `HashJoinExecutor` yet
`ensure:hash_join` (parsed in `tools/sqllogictest/sqllogictest.cpp:98`) asserts the **query plan** contains a HashJoin node — it's a plan-shape check, not just a result check. That shape is only ever produced by `Optimizer::OptimizeNLJAsHashJoin`, called from `OptimizeCustom` (`optimizer_custom_rules.cpp:26`), which is **still a full stub** (`return plan;`, `nlj_as_hash_join.cpp` unchanged all session). Checked and ruled out both possible shortcuts: the `force_starter_rule_` path (`optimizer.cpp:21-27`) doesn't include hash-join conversion at all, and there's no standalone C++ unit test that builds a `HashJoinPlanNode` directly bypassing the optimizer. **So every query in p3.14/p3.15 will fail at the `ensure:hash_join` check regardless of whether the executor is correct** — running them now would give a false signal either way.
- **Next session, first thing**: implement `nlj_as_hash_join.cpp`. Start minimal — just a single `col = col` equality is enough to unblock testing; the full AND-conjunction flattening (multi-key case, tested later in the same file around line 528+) can come after. Mirror `seqscan_as_indexscan.cpp`'s structure: recurse children → if node is `NestedLoopJoin` and predicate is `ComparisonType::Equal` between two `ColumnValueExpression`s → route each side via `GetTupleIdx()` (0=left,1=right) into key vectors → build `HashJoinPlanNode` passing `nlj_plan.GetLeftPlan()`/`GetRightPlan()` straight through as children (same pattern as `nlj_as_index_join.cpp:84`) → else return NLJ unchanged.
- **First test to run once unblocked**: NOT the whole file — just `p3.14-hash-join.slt:19-31` (`select * from test_simple_seq_1 s1 inner join test_simple_seq_2 s2 on s1.col1 = s2.col1;`, single equi-condition INNER join, 10 rows). Smallest smoke test before LEFT join (line 43), multi-key AND conjunctions (line 528+), and the 10,000-row cross-join stress case (line 281) later in the same file.
...
**Still open, low priority, not yet fixed**: NULL-key right tuples are still inserted into `cached_right_tuples_` unfiltered instead of being skipped before insertion — harmless because `HashKey::operator==` already makes any NULL-containing key compare unequal to everything (including itself), so it can never be matched; just wastes a bit of map space. Not worth fixing before the optimizer blocker.
...
- **Tests**: p3.14-hash-join.slt, p3.15-multi-way-hash-join.slt — **cannot pass yet, see blocker above**. Run once unblocked: `build/bin/bustub-sqllogictest <test.slt> --verbose -d --in-memory`. Build command confirmed working this session: `cd build && make -j$(nproc) sqllogictest` (or `cmake --build . --target sqllogictest -j$(sysctl -n hw.ncpu)` on macOS where `nproc` doesn't exist).
