...
### HashJoin — DESIGN LOCKED, SCAFFOLDING IN PLACE, LOGIC NOT YET WRITTEN (resume here)
...
- **Current code state (verified from source 2026-07-13)**:
  - `hash_join_executor.h` ALREADY declares: `plan_`, `left_child_`, `right_child_`, `std::vector<std::vector<page_id_t>> left_pages_` / `right_pages_` (dim0 = partition index, dim1 = list of page_ids in that partition), `std::unordered_map<HashKey, std::vector<Tuple>> right_tuples_` (per-partition build map, NOT whole table), `struct HashKey { std::vector<Value> key_values_; /* TODO operator== */ }`, `static constexpr uint16_t MEM_LIMIT_FOR_TUPLES_PARTITION = 4096;` (already 4096, the `4906` in old notes was stale), and helper decl `void RecursiveBuildHashParition();`.
  ...
  - STILL TODO: `HashKey::operator==` + `std::hash<HashKey>` (file scope, before the map); the partition/recursion body; the `Next()` build+stream-probe state machine; whole `nlj_as_hash_join.cpp`.
